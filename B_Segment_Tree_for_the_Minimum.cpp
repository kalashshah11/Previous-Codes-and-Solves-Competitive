...
struct segmentTree{
	...
	void build(vector<ll>& a,int v,int tl,int tr){
		if (tl == tr) {
			...
			value[v] = min(value[v*2],value[v*2+1]); // merge function minimum , maximum , xor 
		}
	}
	ll sum(int v, int tl, int tr, int l, int r) {
		...
		int tm = (tl + tr) / 2;
		return min(sum(v*2, tl, tm, l, min(r, tm)) ,sum(v*2+1, tm+1, tr, max(l, tm+1), r)); // merge function
	}
	...
	void update(int v, int tl, int tr, int pos, ll new_val) {
		if (tl == tr) {
			...
			value[v] = min(value[v*2] , value[v*2+1]); // merge
		}
	}
};
